...
function solution(info, query) {
  ...
  info.forEach((v) => {
    ...
    for (let i = 0; i <= list.length; i++) {
      combiList.push(...combination(list.slice(0, -1), i))
    }
    for (const combi of combiList) {
      const key = combi.join('')
      const score = list[list.length - 1]
      if (map.has(key)) {
  ...
  query.forEach((v) => {
    const list = v.split(' ').filter((v) => v !== 'and' && v !== '-')
    const condition = list.slice(0, -1).join('')
    const filtered = (map.get(condition) ?? []).filter((v) => v >= Number(list[list.length - 1]))
